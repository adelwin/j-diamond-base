...
import org.si.diamond.base.mail.authenticator.SMTPAuthenticator;
 ...
 * <code> 
 * 	MailSession mailSession = new MailSession("Session Unique Name", "host.smtp.com", null); 
 * 	mailSession.connect("adelwin", "MailPassword0"); 
 ...
 * 	MailSession mailSessionEOD = new MailSession("Session Unique Name 01"); 
 * 	mailSessionMap.put(mailSessionEOD.getIdentifier(), mailSessionEOD); 
 * 	MailSession mailSessionIOD = new MailSession("Session Unique Name 02"); 
...
public class MailSession implements IMailSession {
	 ...
	 * this is the blank constructor to support spring. 
	 ...
	 * @author ahandoyo
	 * @param _identifier
	 * @param _mailConfigurationProperties
	 * @throws org.si.diamond.base.mail.exception.BaseMailException
	 */
	public MailSession(String _identifier, Properties _mailConfigurationProperties) throws BaseMailException {
		try {
			...
			this.smtpHost = this.mailConfigurationProperties.getProperty("mail.smtp.host");
			logger.debug("MailSession(String, Properties) :: smtp host = " + this.smtpHost);

			this.smtpPort = this.mailConfigurationProperties.getProperty("mail.smtp.port");
			...
			logger.info("MailSession(String, Properties, Authenticator) :: setting transport type");
			if (this.mailConfigurationProperties.getProperty("mail.smtp.transport.static") == null || this.mailConfigurationProperties.getProperty("mail.smtp.transport.static").trim().length() == 0) {
				this.setTransportStaticType(TRANSPORT_STATIC_DEFAULT);
			} else {
				this.setTransportStaticType(this.mailConfigurationProperties.getProperty("mail.smtp.transport.static"));
			}
	...
	public MailSession(String _identifier, Properties _mailConfigurationProperties, Authenticator authenticator) throws BaseMailException {
		try {
			...
			this.smtpUserLogin = ((SMTPAuthenticator) authenticator).getUserName();
			this.smtpPassword = ((SMTPAuthenticator) authenticator).getPassword();
	 ...
	 * @author ahandoyo
	 * @param _identifier
	 * @param _mailConfigurationPropertiesFileName
	 * @param authenticator
	 * @throws org.si.diamond.base.mail.exception.BaseMailException
	 */
	...
	public int transportNonStaticSend(MimeMessage mailMessage) throws BaseMailException {
		...
		try {
			if (!this.mailTransport.isConnected()) {
				...
				this.connect(this.getSmtpUserLogin(), this.getSmtpPassword());
			}
